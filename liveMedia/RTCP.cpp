...
void RTCPInstance::incomingReportHandler1() {
    do {
...
#endif
            fNumBytesAlreadyRead = 0; // for next time,jmain
            break;
        }
        Boolean readResult = fRTCPInterface.handleRead(&fInBuf[fNumBytesAlreadyRead], maxRTCPPacketSize - fNumBytesAlreadyRead, numBytesRead, fromAddress, packetReadWasIncomplete);
        ...
        { // normal case: We've read the entire packet 
...
void RTCPInstance::processIncomingReport(unsigned packetSize, struct sockaddr_in const& fromAddress)
{
    do {
