...
{ 
  ...
  else 
  {
    ...
    FramedSource* mediaSource  = createNewStreamSource(clientSessionId, streamBitrate); //创建FramedSource,jamin
  ...
  //RTSPClientSession中的streamstates在ReuseFirstSource时也不能准确找出client端的端口等信息。jamin
	       ...
	       void* serverRequestAlternativeByteHandlerClientData) 
{
  ...
  else 
  {
    ...
    if (fRTCPInstance != NULL) 
    {
      fRTCPInstance->setSpecificRRHandler(dests->addr.s_addr, dests->rtcpPort, rtcpRRHandler, rtcpRRHandlerClientData);
    }
  }
