...
{
  int ourSocket = setUpOurSocket(env, ourPort);  //建立一个TCP连接，监听连接，jamin
...
Boolean RTSPServer::setUpTunnelingOverHTTP(Port httpPort) 
{
  ...
  if (fHTTPServerSocket >= 0) 
  {
    ...
    envir().taskScheduler().turnOnBackgroundReadHandling(fHTTPServerSocket,(TaskScheduler::BackgroundHandlerProc*)&incomingConnectionHandlerHTTP, this);
    return True;
  }
...
int RTSPServer::setUpOurSocket(UsageEnvironment& env, Port& ourPort)
{
  ...
  do {
    ...
    ourSocket = setupStreamSocket(env, ourPort);//jamin,创建socket
    if (ourSocket < 0) 
        break;
...
void RTSPServer::incomingConnectionHandler(int serverSocket) 
{
  ...
  int clientSocket = accept(serverSocket, (struct sockaddr*)&clientAddr, &clientAddrLen);//接收客户端 RTSP连接,jamin
  if (clientSocket < 0) 
  {
  ...
  makeSocketNonBlocking(clientSocket);//使用FIONBIO，对套接字设置非阻塞模式。

  increaseSendBufferTo(envir(), clientSocket, 50*1024);//设置套接字参数，指定发送缓冲区大小
  fConnectSocket++;

#ifdef _DEBUG
  envir() << "accept()ed connection from " << AddressString(clientAddr).val()<<fConnectSocket<<"\n";//jamin
#endif
  envir() << "\naccept()ed connection from " << AddressString(clientAddr).val() << fConnectSocket << "\n";//jamin

  // Create a new object for handling this RTSP connection:
  //创建一个信息RTSP-Client对象，为一个新的RTSP客户端连接//jamin
    ...
    fRecursionCount(0), fOurSessionCookie(NULL) 
{
  ...
  fOurServer.fClientConnections->Add((char const*)this, this);//加入连接对象到 哈希表,将客户端Socket对象加入到Select模型中jamin
  
  // Arrange to handle incoming requests:
  resetRequestBuffer();//初始化请求缓冲区
  //下面加入Select模型，任务调度，socket操作,读取客户端的请求,jamin
...
void RTSPServer::RTSPClientConnection::incomingRequestHandler1() 
{
  ...
  handleRequestBytes(bytesRead);//解析，处理读取得到的数据，jamin
}
  ...
  do 
  {
      ...
      if (parseSucceeded)
      {
          ...
          if (requestIncludedSessionId)
          {
              ...
              if (clientSession != NULL)
                  clientSession->noteLiveness(); //超时设置,jamin
          }
                ...
                || strcmp(cmdName, "SET_PARAMETER") == 0)
          {
              if (clientSession != NULL)
              {
                  clientSession->handleCmd_withinSession(this, cmdName, urlPreSuffix, urlSuffix, (char const*)fRequestBuffer);//参数判断是否进一步执行，分别判断是哪一个命令
              }
              else
              {
                  handleCmd_sessionNotFound();
      ...
      else
      {
          ...
          *fLastCRLF = '\0'; // temporarily, for parsing
          parseSucceeded = parseHTTPRequestString(cmdName, sizeof cmdName, urlSuffix, sizeof urlPreSuffix, sessionCookie,
...
void RTSPServer::RTSPClientSession::handleCmd_SETUP(RTSPServer::RTSPClientConnection* ourClientConnection,char const* urlPreSuffix, char const* urlSuffix, char const* fullRequestStr) 
{
  ...
  do {
      ...
      { // normal case
          for (streamNum = 0; streamNum < fNumStreamStates; ++streamNum)
          {
              ...
              if (subsession != NULL && strcmp(trackId, subsession->trackId()) == 0) break;//找到对应的track，jamin
          }
...
void RTSPServer::RTSPClientSession::handleCmd_PLAY(RTSPServer::RTSPClientConnection* ourClientConnection, ServerMediaSubsession* subsession, char const* fullRequestStr)
{
  ...
  for (i = 0; i < fNumStreamStates; ++i) 
  {
    if (subsession == NULL /* means: aggregated operation */ || subsession == fStreamStates[i].subsession) 
    {
      ...
      fStreamStates[i].subsession->startStream(fOurSessionId,
					       ...
					       RTSPServer::RTSPClientConnection::handleAlternativeRequestByte, ourClientConnection);//开启流
...
RTSPServer::RTSPClientConnection* RTSPServer::createNewClientConnection(int clientSocket, struct sockaddr_in clientAddr) 
{
...
void RTSPServer::RTSPClientSession::noteLiveness() 
{
    if (fOurServer.fReclamationTestSeconds > 0)
    {
        envir().taskScheduler().rescheduleDelayedTask(fLivenessCheckTask,fOurServer.fReclamationTestSeconds*1000000,(TaskFunc*)livenessTimeoutTask, this);
    }
}
