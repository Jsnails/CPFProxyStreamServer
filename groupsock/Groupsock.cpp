...
Boolean Groupsock::handleRead(unsigned char* buffer, unsigned bufferMaxSize, unsigned& bytesRead,struct sockaddr_in& fromAddress) 
{
  ...
  if (!wasLoopedBackFromUs(env(), fromAddress)) 
  {
    ...
    numMembers =outputToAllMembersExcept(NULL, ttl(), buffer, bytesRead,fromAddress.sin_addr.s_addr);
...
#ifdef _DEBUG
  //fprintf(stderr, "\nGroupsock::handleRead:\nread:%d bytes from%s;relayed to:%d members \n", bytesRead, buffer, AddressString(fromAddress).val(), numMembers); //jamin
#endif
  return True;
}
