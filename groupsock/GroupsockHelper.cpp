...
#if defined(__WIN32__) || defined(_WIN32)
  unsigned long arg = 1;
  return ioctlsocket(sock, FIONBIO, &arg) == 0;//控制套接口的模式,jamin
...
static unsigned increaseBufferTo(UsageEnvironment& env, int bufOptName,int socket, unsigned requestedSize)
{
  ...
  unsigned curSize = getBufferSize(env, bufOptName, socket);//获取套接口的选项
  ...
  {
    SOCKLEN_T sizeSize = sizeof requestedSize;
    if (setsockopt(socket, SOL_SOCKET, bufOptName,(char*)&requestedSize, sizeSize) >= 0) //设置套接口的选项
    {
      // success
